		...
		<div class="col-sm-12 col-xl-6">
			<div class="bg-light rounded h-100 p-4">
				<c:if test="${user.acc_id !=null }">
					...
					<i>${error}</i>
					<form:form id="myForm" method="POST" action="/quan-tri/tai-khoan/nguoi-dung/update" modelAttribute="user" >
						...
						<a href="/quan-tri/tai-khoan/nguoi-dung/list"><button style="margin-left: 100px !important" type="button" class="btn btn-danger m-2">Quay lại</button></a>
					</form:form>
				</c:if>
				<c:if test="${admin.acc_id != null }">
					<h6 class="mb-4">Edit admin account</h6>
					<i>${error}</i>
					<form:form id="myForm" method="POST" action="/quan-tri/tai-khoan/admin/update" modelAttribute="admin" >
						<form:input path="acc_id" type="hidden"/>
						<div class="mb-3">
							<label for="InputSupName" class="form-label">Name</label>
							<form:input path="name" type="text" class="form-control" id="InputSupName" required="required"/>
						...
						<button type="submit" class="btn btn-primary">Save</button>
						<a href="/quan-tri/trang-chu"><button style="margin-left: 100px !important" type="button" class="btn btn-danger m-2">Quay lại</button></a>
					</form:form>
				</c:if>
			</div>
		</div>
